...
function from_prior!(particle::Particle)
	...
	## Midpoint can be anywhere on a players score
...
function perturb!(particle::Particle)
	...
	## Alter K - Gaussian strength parameter
	if(i == 4)
		sig = 2/12
	...
	## Alter Gaussian width parameter
	if(i == 5)
		## Standard deviation
		sig = 20/12
...
function effective_average(particle::Particle, data::Array{Float64} = data)
	...
	k = particle.params[4]
	w = particle.params[5]
	mid = particle.params[6]
	gaussian = exp(-k * exp(-1/(2 * w^2) * (data[:, 1] - mid).^2))
	return((mu2 + (mu1 - mu2) * exp(-data[:, 1] / L)) .* gaussian)
end
